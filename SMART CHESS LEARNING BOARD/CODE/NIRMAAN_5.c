...
// Hardware pins (Mega)
const uint8_t rowPins[ROWS] = {39,41,43,45,47,49,51,53};
const uint8_t colPins[COLS] = {4,5,6,7,8,9,10,11};
      ...
      for (int row = 0; row < 5; row++) {
        for (int col = 0; col < charWidth; col++) {
          ...
          if (ledCol >= 0 && ledCol < COLS && ledRow >= 0 && ledRow < ROWS) {
            ...
            if (pixel) {
              // Cycle through Indian flag colors
              CRGB color;
              int colorPhase = (charIdx + (offset / 2)) % 3;
              if (colorPhase == 0) color = saffron;
              else if (colorPhase == 1) color = white;
              else color = greenC;
